...
export class OfferPage {
  ...
  async getOfferStatus(number: number) {
    return await this.page.locator(`div:has([data-testid="find-offerlist-offerbox"]):nth-child(${number}) + div div`).first().getAttribute('data-status');
  }

  async countConfirmedOffers() {
    await this.page.waitForSelector('[data-status="loading"]', {state: 'hidden' })
    return await this.page.locator('[data-testid="find-offerlist-offerbox"]:has(a[data-testid="find-offerlist-offerbox-cta"])').count()
  }
